...
import "../openPurchaseOrder/styles/style.css";
import AutoSugession from "../autoSugessionInput/autoSugessionInput";
import Input from "@material-ui/core/Input";
import colorIcon from "../openPurchaseOrder/color-icon.png";
  ...
  handleClose = () => {
    ...
    if (this.state.order.length) {
      if (this.state.checkReciept) {
        ...
        store.dispatch({
          type: "CLOSE_SALE_DIALOG"
        });
      }
    } else {
      store.dispatch(
        removeTableByStoringAction({ table: this.props.openTable.tableName })
      );
  ...
  addOrder = () => {
    ...
    filterProducts = this.props.openTable.brands.map(item => {
      ...
      return item.items.find(allitem => {
        return (
          allitem.brand === splitBarcode[0] &&
          allitem.size === splitBarcode[1] &&
          allitem.color.name === splitBarcode[2]
        );
      });
    });
    ...
    if (this.state.specialDiscountMode) {
      ...
      this.state.barcode && newProducts
        ? order.push({
            ...
            singlePrice: Math.round(
              newProducts.singlePrice -
                (newProducts.singlePrice * this.state.specialDiscount) / 100
            )
          })
        : this.setState({
            order: []
    ...
    if (this.state.barcode && newProducts) {
      ...
      if (this.state.checked) {
        tax = Math.round(
          0.16 * (this.state.totalBill + newProducts.singlePrice)
        );
        let grandTotal =
          this.state.totalBill + newProducts.singlePrice + tax - discountedBill;
        this.setState({
          order: order,
          totalBill: this.state.totalBill + newProducts.singlePrice,
      ...
      if (this.state.specialDiscountMode) {
        this.setState({
          grandTotal: Math.round(
            totalBill - (totalBill * this.state.specialDiscount) / 100
          )
        });
      } else {
        this.setState({
  ...
  updateBill = args => {
    if (this.state.order.length) {
      ...
      this.setState({
        ...
        totalBill: lTotal,
        grandTotal: Math.round(
          (lTotal - discountedBill) * tax * serviceCharges
        ),
            ...
            >
              ...
              <DialogContent className="table-dialogContent">
                <DialogContentText>
                  ...
                  {this.state.barcode[this.state.barcode.length - 1] === "_"
                    ? this.addOrder()
                    : null}
              ...
              <DialogActions>
                ...
                {!this.props.hideReceipt && this.state.order.length ? (
                  <div>
                    <Button
                      className="mr-10"
                      disabled={
